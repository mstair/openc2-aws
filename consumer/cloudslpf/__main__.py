...
if len(sys.argv) != 3:
    print("Usage: cloudslpf <configuration files directory> <cloud>")
    sys.exit(1)
...
logging_config_path = os.path.join(config_dir, CloudSlpf.LOGGING_CONFIG_FILE)
...
with CloudSlpf(sys.argv[1],cloud) as app:
    try:
        # Run the application
        app.run()
        running = True
